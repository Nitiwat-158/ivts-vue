...
test('IVTS bootstrap script provisions permissions and assigns the configured IAM account', async function () {
  const scriptPath = path.resolve(__dirname, '../scripts/bootstrap-ivts-permissions.js');
  const execution = await execFileAsync(process.execPath, [scriptPath], {
    ...
    env: Object.assign({}, process.env, {
      ...
      PROJECT_PERMISSION_ROOT_PATH: '/ivts/security/permission',
      PROJECT_PERMISSION_PATHS: '/ivts/security/permission,/dashboard,/accounts/directory',
  ...
  assert.ok(payload.paths.includes('/ivts/security/permission'));
