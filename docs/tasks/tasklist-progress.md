...
| ivts-STREAM-006 | CCTV Live Video: Enhance HLS proxy & CORS fallback for live video stream | Full-stack | AI | none | done | 100 | Enhanced proxyHlsStream with MediaMTX upstream proxying & CORS; added automatic proxy fallback in CameraView.vue | `cctv.js`, `CameraView.vue` | `node --check PASS` | none | none | Live video stream playback with fallback |
| ivts-STREAM-011 | CCTV Live Video: Fix CameraView.vue Vue template v-if="loading" DOM mounting bug | Full-stack | AI | none | done | 100 | Restructured CameraView.vue template to mount video element while online, putting loading overlay as absolute sibling | `CameraView.vue` | code inspection | none | none | Mounted video ref in DOM with absolute overlay |
| ivts-MAP-001 | Mobile App: Fix OpenStreetMap 403 Tile Access Blocked error | Mobile | AI | none | done | 100 | Updated userAgentPackageName to th.ac.mfu.ivts & configured CartoDB Voyager tile provider | `location_screen.dart` | `flutter analyze PASS` | none | none | Compliant map tile layer |
