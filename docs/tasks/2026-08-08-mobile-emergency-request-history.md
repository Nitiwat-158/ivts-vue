...
| ivts-MERH-003 | Update `emergency_request_screen.dart` & `locale_provider.dart` | Mobile | AI | ivts-MERH-002 | done | 100 | Flutter payload & localization updated | `emergency_request_screen.dart`, `locale_provider.dart` | `flutter analyze` PASS | none | — | Flutter app changes |
| ivts-MERH-004 | Run verification tests (backend syntax check & flutter analyze) | QA | AI | ivts-MERH-003 | done | 100 | Syntax check & flutter analyze verified | `backend-node/`, `user-mobile-application/` | `node --check` PASS, `flutter analyze` 0 errors | none | — | Verification evidence |
